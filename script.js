...
// -----------------------------------------
// 3) DOWNLOAD PDF USING html2canvas and jsPDF
// -----------------------------------------
function downloadPDF() {
    const formContainer = document.getElementById("form-container");

    // Store Original Inputs and Replace with Text
    ...
    // Find the Split Point ("2. HANDS-ON SKILLS" title)
    const splitTitle = Array.from(document.querySelectorAll(".metric-title")).find(el => el.textContent.trim() === "2. HANDS-ON SKILLS");
    const splitContent = splitTitle.nextElementSibling; // Get the <p> element with the details

    // Create the new parent div
    const handsOnSkillsSection = document.createElement("div");
    handsOnSkillsSection.id = "hands-on-skills-section";

    // Insert the new div before the title
    splitTitle.parentNode.insertBefore(handsOnSkillsSection, splitTitle);

    // Move the title and content into the new div
    handsOnSkillsSection.appendChild(splitTitle);
    handsOnSkillsSection.appendChild(splitContent);

    const splitPosition = handsOnSkillsSection.getBoundingClientRect().top;
    ...
    html2canvas(clonedFormContainer, {
        scale: 2,
        useCORS: true,
        height: splitPosition - clonedFormContainer.getBoundingClientRect().top,
        windowWidth: formContainer.offsetWidth, // Ensure consistent width
        windowHeight: splitPosition - clonedFormContainer.getBoundingClientRect().top // Ensure consistent height
    }).then(canvas1 => {
        const imgData1 = canvas1.toDataURL("image/png");

        // Remove the First Part from the Clone
        const splitParent = document.getElementById("hands-on-skills-section");
        while (clonedFormContainer.firstChild !== splitParent) {
            clonedFormContainer.removeChild(clonedFormContainer.firstChild);
        }
        ...
        html2canvas(clonedFormContainer, {
            ...
            useCORS: true,
            windowWidth: formContainer.offsetWidth, // Ensure consistent width
            windowHeight: clonedFormContainer.offsetHeight // Ensure consistent height
        }).then(canvas2 => {
            ...
            pdf.addImage(imgData1, "PNG", 0, 10, imgWidth1, imgHeight1);

            // Add the Second Page
            pdf.addPage();
            const imgWidth2 = 210;
            const imgHeight2 = (canvas2.height * imgWidth2) / canvas2.width;
            pdf.addImage(imgData2, "PNG", 0, 10, imgWidth2, imgHeight2);

            // Save the PDF
            pdf.save("Employee_Performance_Evaluation.pdf");

            // Clean Up
            document.body.removeChild(clonedFormContainer);

            // Restore Original Inputs
            originalInputs.forEach(inputData => {
                const textDiv = inputData.parent.childNodes[0];
                inputData.parent.removeChild(textDiv);
                if (inputData.nextSibling) {
                    inputData.parent.insertBefore(inputData.element, inputData.nextSibling);
                } else {
                    inputData.parent.appendChild(inputData.element);
                }
                inputData.element.value = inputData.value;
            });
        });
    });
}
